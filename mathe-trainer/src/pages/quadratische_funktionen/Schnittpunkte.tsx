...
const Schnittpunkte: React.FC<SchnittpunkteProps> = ({ initialTaskType = 'line-parabola' }) => {
    ...
    // Wie formatLinearTerm, aber für einen ax²-Term (z. B. in der
    // Äquivalenzumformung beim Gleichsetzen zweier Parabeln).
    const formatQuadraticTerm = (value: number) => {
        if (value === 0) return "";
        if (value === 1) return "+ x^2";
        if (value === -1) return "- x^2";
        if (value > 0) return `+ ${value}x^2`;
        return `- ${Math.abs(value)}x^2`;
    };
            ...
            return (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="mb-4">
                        ...
                        <BlockMath math={`${m}x ${formatConstant(t)} = ${a}x^2 ${formatLinearTerm(b)} ${formatConstant(c)} \\quad \\Big| ${formatLinearTerm(-m)} ${formatConstant(-t)}`} />
            ...
            return (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="mb-4">
                        ...
                        <BlockMath math={`${a1}x^2 ${formatLinearTerm(b1)} ${formatConstant(c1)} = ${a2}x^2 ${formatLinearTerm(b2)} ${formatConstant(c2)} \\quad \\Big| ${formatQuadraticTerm(-a2)} ${formatLinearTerm(-b2)} ${formatConstant(-c2)}`} />
