      ...
      <main className="flex-1 w-full mx-auto px-4 py-8 grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 justify-center justify-items-center max-w-6xl">
        {aufgaben.map((a) => (
          <Link
            key={a.title}
            to={a.path}
            className="bg-white rounded-2xl shadow-md border border-slate-200 flex flex-col justify-center items-center p-6 hover:shadow-xl transition-shadow no-underline text-inherit text-center max-w-sm w-fit"
          >
