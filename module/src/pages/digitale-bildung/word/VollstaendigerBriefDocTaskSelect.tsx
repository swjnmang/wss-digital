...
export default function VollstaendigerBriefDocTaskSelect() {
  const navigate = useNavigate();

  const goToTask = (taskId: string) => {
    navigate(`/digitale-bildung/word/geschaeftsbrief/vollstaendiger-brief-doc/${taskId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-gradient-to-br from-slate-800 to-slate-700 text-white py-8 px-4 text-center relative">
        ...
        <h1 className="text-2xl font-bold">📄 Vollständiger Geschäftsbrief (Word-Editor)</h1>
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto p-6 flex flex-col gap-6">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 text-sm text-blue-900 leading-relaxed">
          ...
          <p>
            Diese Variante läuft in einem echten, frei editierbaren Texteditor direkt im Browser (wie Word) – ohne
            Datei-Upload oder -Download. Anschriftenfeld, Infoblock, Betreff, Anrede, Brieftext sowie Grußformel und
            Unterschrift werden in einem einzigen Dokument bearbeitet und geprüft.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4">
          {WORD_DOKUMENT_TASKS.map((task) => (
            <button
              key={task.id}
