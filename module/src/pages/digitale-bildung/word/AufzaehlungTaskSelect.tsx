...
export default function AufzaehlungTaskSelect() {
  const navigate = useNavigate();

  const goToTask = (taskId: string) => {
    navigate(`/digitale-bildung/word/geschaeftsbrief/aufzaehlung/${taskId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-gradient-to-br from-slate-800 to-slate-700 text-white py-8 px-4 text-center relative">
        ...
        <h1 className="text-2xl font-bold">🔢 Aufzählung &amp; Nummerierung</h1>
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto p-6 flex flex-col gap-6">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 text-sm text-blue-900 leading-relaxed">
          ...
          <p>
            Du liest jeweils ein Beispiel und beantwortest Fragen zu Leerzeilen, Einzug und Satzzeichen bei
            Aufzählungen und Nummerierungen nach DIN 5008.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {AUFZAEHLUNG_TASKS.map((task) => (
            <button
              key={task.id}
            ...
            >
              ...
              <p className="text-sm text-slate-500 line-clamp-2">{task.intro}</p>
            </button>
          ))}
        </div>
      </main>
